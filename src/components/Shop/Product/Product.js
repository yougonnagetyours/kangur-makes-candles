    ...
    <div className="">
      <Link to={`/${product.id}`}>
        <div className="border border-black shadow-xl">
          <img className="" src={product.image.url} alt={product.name} />
        </div>
      </Link>
      <div className="flex justify-around mt-6 mb-2">
        <p className="text-center text-base sm:text-sm tracking-widest">
          {product.name}
        </p>
      </div>
      ...
      <div
        className={`flex justify-around mx-auto mt-5 mb-2 py-2 border-2 border-black cursor-pointer w-auto sm:hidden`}
